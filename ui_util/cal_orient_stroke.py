...
class orient(nn.Module):
    ...
    def calOrientation(self, image, mask=None):
        ...
        for iOrient in range(self.numKernels):
            ...
            response = F.conv2d(
                image, filterKernel, stride=self.stride, padding=self.padding
            )
        ...
        orientTwoChannel = torch.cat(
            [torch.sin(2 * orientTensor), torch.cos(2 * orientTensor)], dim=1
        )
    ...
    def convert_orient_to_RGB_test(self, input, label):
        ...
        label = label.float()
        input = input * label
        out_r = torch.unsqueeze(
            input[1, :, :] * label[0, ...] + (1 - label[0, ...]) * -1, 0
        )
        out_g = torch.unsqueeze(
            input[0, :, :] * label[0, ...] + (1 - label[0, ...]) * -1, 0
        )
        out_b = torch.unsqueeze(
            input[0, :, :] * 0 * label[0, ...] + (1 - label[0, ...]) * -1, 0
        )
        # print(out_b.shape)
    ...
    def stroke_to_orient(self, stroke_mask):
        ...
        # vis
        orient_rgb = self.convert_orient_to_RGB_test(
            orient_tensor[0, ...], stroke_mask_tensor[0, ...]
        )  # [3, h, w]
        orient_numpy = (
            (np.transpose(orient_rgb.cpu().numpy(), (1, 2, 0)) + 1) / 2.0 * 255.0
        )
