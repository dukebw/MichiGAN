...
class Pix2pixDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.use_ig and not self.opt.no_orientation:
            transform_orient_rgb = get_transform(
                self.opt, params, method=Image.NEAREST, normalize=False
            )
            if orient_random_param < orient_random_th:
                ...
                orient_rgb = trans_orient_to_rgb(
                    np.array(orient_rgb), np.array(label), np.array(orient_mask)
                )
