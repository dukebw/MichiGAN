...
def pad_zeros(input, pad_th):
    ...
    if len(size) == 2:
        ...
        pad_img[
            int(pad_th / 2) : int(pad_th / 2) + H, int(pad_th / 2) : int(pad_th / 2) + W
        ] = img
    else:
        H, W, C = size[0], size[1], size[2]
        pad_img = np.zeros((H + pad_th, W + pad_th, C))
        pad_img[
            int(pad_th / 2) : int(pad_th / 2) + H,
            int(pad_th / 2) : int(pad_th / 2) + W,
            :,
        ] = img
...
def single_inference_dataLoad(opt):
    ...
    subset = opt.subset
    label_ref_dir = (
        base_dir + "/" + subset + "_labels/" + opt.inference_ref_name + ".png"
    )
    label_tag_dir = (
        base_dir + "/" + subset + "_labels/" + opt.inference_tag_name + ".png"
    )
    orient_tag_dir = (
        base_dir
        + "/"
    ...
    orient_ref_dir = (
        ...
        + subset
        + "_dense_orients/"
        + opt.inference_orient_name
        + "_orient_dense.png"
    )
    orient_mask_dir = (
        base_dir + "/" + subset + "_labels/" + opt.inference_orient_name + ".png"
    )
    image_ref_dir = (
        base_dir + "/" + subset + "_images/" + opt.inference_ref_name + ".jpg"
    )
    image_tag_dir = (
        base_dir + "/" + subset + "_images/" + opt.inference_tag_name + ".jpg"
    )
...
def generate_noise(width, height):
    ...
    while width >= 8 and height >= 8:
        noise += (
            cv2.resize(
                np.random.normal(
                    loc=0.5, scale=0.25, size=(int(height), int(width), 3)
                ),
                dsize=(noise.shape[0], noise.shape[1]),
            )
            * weight
        )
        weightSum += weight
...
def get_transform(
    opt, params, method=Image.BICUBIC, normalize=True, toTensor=True, color=False
):
    transform_list = []
