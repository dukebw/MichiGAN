...
class Pix2PixModel(torch.nn.Module):
        ...
        if mode == "generator":
            ...
            with torch.no_grad():
                if self.opt.use_ig:
                    ...
                    ) = self.inpainting_orient(hole, orient_rgb, noise, hair_mask)
                    inpainted_orient_rgb = np.uint8(
                        inpainted_orient_rgb[0, ...].permute(1, 2, 0).cpu().numpy()
                        * 255
                    )
                    fake_image, _, blend_image = self.generate_fake(
                        input_ref,
                        image_ref,
    ...
    def initialize_networks(self, opt):
        ...
        netE = (
            networks.define_E(opt) if opt.use_vae else None
        )  # this is for original spade network
        netIG = (
            networks.define_IG(opt) if opt.use_ig else None
        )  # this is the orient inpainting network
        netSIG = (
            networks.define_SIG(opt) if opt.use_stroke else None
        )  # this is the stroke orient inpainting network
        netFE = (
            networks.define_FE(opt) if opt.use_instance_feat else None
        )  # this is the feat encoder from pix2pixHD
    ...
    def compute_generator_loss(
        ...
        if not self.opt.no_gan_loss:
            ...
            G_losses["GAN"] = self.criterionGAN(
                pred_fake, True, for_discriminator=False, label=label_tag
            )
        ...
        if self.opt.curr_step == 1:
            ...
            if not self.opt.no_vgg_loss:
                if ref_is_tag:
                    ...
                    if self.opt.use_blender:
                        G_losses["VGG"] = (
                            self.criterionVGG(blend_image, image_tag, label_tag)
                            * self.opt.lambda_vgg
                        )
                    else:
                        G_losses["VGG"] = (
                            self.criterionVGG(fake_image, image_tag, label_tag)
                            * self.opt.lambda_vgg
                        )
        ...
        if self.opt.unpairTrain and self.opt.curr_step == 2:
            if self.opt.use_blender:
                hair_avg_lab_loss = self.criterionHairAvgLab(
                    blend_image,
                    fake_image.detach(),
                    torch.unsqueeze(input_tag[:, 1, :, :], 1),
                    torch.unsqueeze(input_tag[:, 1, :, :], 1),
    ...
    def compute_discriminator_loss(
        ...
        D_losses["D_Fake"] = self.criterionGAN(
            pred_fake, False, for_discriminator=True, label=label_tag
        )
        D_losses["D_real"] = self.criterionGAN(
            pred_real, True, for_discriminator=True, label=label_tag
        )
    ...
    def inpainting_orient(self, hole, orient_rgb, noise, mask):
        ...
        if self.opt.crop_size != 256:
            output = F.interpolate(
                output, size=(self.opt.crop_size, self.opt.crop_size), mode="nearest"
            )
    ...
    def inpainting_stroke_orient(
        self, hole, orient_rgb, noise, mask, stroke, stroke_mask, mask_orient_rgb
    ):
    ...
    def zeros_padding(self, input):
        ...
        pad_output[
            :, :, int(th / 2) : int(th / 2) + H, int(th / 2) : int(th / 2) + W
        ] = input
        return pad_output
    ...
    def discriminate(self, input_tag, fake_image, real_image, orient_mask=None):
        ...
        if not self.opt.use_ig:
            ...
            orient_input = torch.cat(
                [torch.sin(2 * orient_mask), torch.cos(2 * orient_mask)], dim=1
            )  # [n,2,h,w]
            orient_input = orient_input * torch.unsqueeze(
                input_tag[:, 1, :, :], 1
            )  # hair_mask==seg[:,1,:,:]
        else:
