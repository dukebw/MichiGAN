    ...
    def forward(self, x, label_ref, label_tag):
        ...
        for b in range(x.size()[0]):
            if self.opt.ref_global_pool:
                tmps = x[b, ...]
                tmps = torch.mean(
                    torch.mean(tmps, dim=1, keepdim=True), dim=2, keepdim=True
                )
            else:
                tmps = x[b, ...] * label_ref[b, ...]
                tmps = torch.sum(
                    torch.sum(tmps, dim=1, keepdim=True), dim=2, keepdim=True
                ) / max(torch.sum(label_ref[b, ...]), 1)
            tmps = tmps.expand_as(x[b, ...])
            outputs_mean[b, ...] = tmps * label_tag[b]
        # resize
        if self.sh != xh:
            outputs_mean = F.interpolate(
                outputs_mean, size=(self.sh, self.sw), mode="nearest"
            )
...
class ImageEncoder3(BaseNetwork):
    ...
    def __init__(self, opt, sw, sh):
        ...
        self.layer2 = PartialConv2d(
            ndf * 1, ndf * 2, kw, stride=2, padding=pw, return_mask=True
        )
        self.norm2 = nn.InstanceNorm2d(ndf * 2, affine=False)
        self.layer3 = PartialConv2d(
            ndf * 2, ndf * 4, kw, stride=2, padding=pw, return_mask=True
        )
        self.norm3 = nn.InstanceNorm2d(ndf * 4, affine=False)
        self.layer4 = PartialConv2d(
            ndf * 4, ndf * 8, kw, stride=2, padding=pw, return_mask=True
        )
        self.norm4 = nn.InstanceNorm2d(ndf * 8, affine=False)
        self.layer5 = PartialConv2d(
            ndf * 8, ndf * 16, kw, stride=2, padding=pw, return_mask=True
        )
    ...
    def forward(self, x, label_ref0, label_tag0):
        ...
        if self.sh != xh:
            outputs_mean = F.interpolate(
                outputs_mean, size=(self.sh, self.sw), mode="bilinear"
            )
...
class BackgroundEncode(BaseNetwork):
    def __init__(self, opt):
        ...
        self.conv1 = ConvBlock(
            3, self.ngf, 7, 1, 3, norm="none", activation="relu", pad_type="reflect"
        )
    ...
    def forward(self, image, mask):
        ...
        back_mask1 = F.interpolate(
            back_mask, size=(int(sh / 2), int(sw / 2)), mode="nearest"
        )
        back_mask2 = F.interpolate(
            back_mask, size=(int(sh / 4), int(sw / 4)), mode="nearest"
        )
        back_mask3 = F.interpolate(
            back_mask, size=(int(sh / 8), int(sw / 8)), mode="nearest"
        )
        back_mask4 = F.interpolate(
            back_mask, size=(int(sh / 16), int(sw / 16)), mode="nearest"
        )

        return (
            [x0, x1, x2, x3, x4],
            [back_mask, back_mask1, back_mask2, back_mask3, back_mask4],
        )
    ...
    def forward(self, image, mask, noise):
        ...
        if self.opt.isTrain:
            if self.opt.random_expand_mask:
                ...
                p = int(k / 2)
                expand_hair_mask = F.max_pool2d(
                    hair_mask, kernel_size=k, stride=1, padding=p
                )
            ...
            if self.opt.expand_mask_be:
                ...
                if self.opt.add_feat_zeros:
                    ...
                    hair_no_pad = hair_mask[
                        :,
                        :,
                        int(th / 2) : int(th / 2) + H,
                        int(th / 2) : int(th / 2) + W,
                    ]
                    expand_hair_no_pad = F.max_pool2d(
                        hair_no_pad, kernel_size=k, stride=1, padding=p
                    )
                    expand_hair_mask[
                        :,
                        :,
        ...
        if self.opt.num_upsampling_layers == "most":
            return (
                [x3, x2, x1, x0, x00],
                [back_mask4, back_mask3, back_mask2, back_mask1, back_mask],
            )
        else:
