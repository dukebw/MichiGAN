        ...
        if self.gan_mode == "original":  # cross entropy loss
            ...
            if not self.opt.remove_background:
                ...
                if for_discriminator:
                    if target_is_real:
                        minval = torch.min(
                            (input - 1) * label1, self.get_zero_tensor(input)
                        )
        ...
        if isinstance(input, list):
            ...
            for pred_i in input:
                ...
                loss_tensor = self.loss(
                    pred_i, target_is_real, for_discriminator, label.detach()
                )
        ...
        for i in range(num_D):  # for each discriminator
            ...
            for j in range(num_intermediate_outputs):  # for each layer output
                if self.opt.remove_background:
                    unweighted_loss = self.L1_loss_mask(
                        pred_fake[i][j], pred_real[i][j].detach(), label.detach()
                    )
                else:
...
class VGGLoss(nn.Module):
    ...
    def L1_loss_mask(self, input, target, label=None):
        ...
        loss = self.criterion_sum(input * label1, target * label1) / (
            label1.sum() * c + 1e-5
        )
        return loss
    ...
    def forward(self, x, y, label=None):
        ...
        for i in range(len(x_vgg)):
            if self.opt.remove_background:
                loss += self.weights[i] * self.L1_loss_mask(
                    x_vgg[i], y_vgg[i].detach(), label.detach()
                )
            else:
...
def gabor_fn(kernel_size, channel_in, channel_out, theta):
    ...
    x = (
        x_0.view(-1, 1).repeat(channel_out, channel_in, 1, ksize).float()
    )  # [channel_out, channelin, kernel, kernel]

    # Rotation
    # don't need to expand, use broadcasting, [64, 1, 1, 1] + [64, 3, 7, 7]
    x_theta = x * torch.cos(theta.view(-1, 1, 1, 1)) + y * torch.sin(
        theta.view(-1, 1, 1, 1)
    )
    y_theta = -x * torch.sin(theta.view(-1, 1, 1, 1)) + y * torch.cos(
        theta.view(-1, 1, 1, 1)
    )
    ...
    gb = torch.exp(
        -0.5
        * (
            x_theta ** 2 / sigma_x.view(-1, 1, 1, 1) ** 2
            + y_theta ** 2 / sigma_y.view(-1, 1, 1, 1) ** 2
        )
    ) * torch.cos(
        2 * math.pi / Lambda.view(-1, 1, 1, 1) * x_theta + psi.view(-1, 1, 1, 1)
    )
...
def DoG_fn(kernel_size, channel_in, channel_out, theta):
    ...
    gb = (
        torch.exp(
            -0.5
            * (
                x_theta ** 2 / sigma_h.view(-1, 1, 1, 1) ** 2
                + y_theta ** 2 / sigma_y.view(-1, 1, 1, 1) ** 2
            )
        )
        / sigma_h
        - torch.exp(
            -0.5
            * (
                x_theta ** 2 / sigma_l.view(-1, 1, 1, 1) ** 2
                + y_theta ** 2 / sigma_y.view(-1, 1, 1, 1) ** 2
            )
        )
        / sigma_l
...
class L1OLoss(nn.Module):
    ...
    def calOrientationGabor(self, image):
        ...
        for iOrient in range(self.numKernels):
            theta = nn.Parameter(
                torch.ones(self.channel_out) * (math.pi * iOrient / self.numKernels),
                requires_grad=False,
            ).cuda()
            GaborKernel = gabor_fn(
                self.kernel_size, self.channel_in, self.channel_out, theta
            )
            GaborKernel = GaborKernel.float()
            response = F.conv2d(
                image, GaborKernel, stride=self.stride, padding=self.padding
            )
        ...
        orientTwoChannel = (
            torch.cat([torch.sin(2 * orientTensor), torch.cos(2 * orientTensor)], dim=1)
            * confidenceTensor
        )
    ...
    def calOrientationDoG(self, image, mask):
        ...
        for iOrient in range(self.numKernels):
            ...
            response = F.conv2d(
                image, DoGKernel, stride=self.stride, padding=self.padding
            )
            resArray.append(response.clone())
    ...
    def forward(self, fake_image0, orientation_label0, input_semantics):
        ...
        if "gabor" in self.mode:
            orientation_fake, confidence = self.calOrientationGabor(
                gray
            )  # [n, 2, h, w]
        else:
            orientation_fake, confidence = self.calOrientationDoG(
                gray, hair_mask
            )  # [n, 2, h, w]
        ...
        if not self.opt.use_ig:
            ...
            orientation_mask = torch.cat(
                [torch.sin(2 * orientation_label), torch.cos(2 * orientation_label)],
                dim=1,
            )
        else:
        ...
        if "gabor" in self.mode:
            ...
            confidence_loss = -torch.sum(torch.log(confidence) * hair_mask) / torch.sum(
                hair_mask
            )
        else:
            ...
            confidence_loss = torch.sum(
                torch.abs(confidence - confidence_gt.detach())
            ) / (torch.sum(hair_mask) + 1e-5)
...
class LabColorLoss(nn.Module):
    ...
    def forward(self, fake, real, mask=None):
        ...
        if self.opt.balance_Lab:
            ...
            lab_loss = self.criterion(
                fake_Lab[:, 1:, :, :], real_Lab[:, 1:, :, :].detach()
            )
...
class HairAvgLabLoss(nn.Module):
    ...
    def cal_hair_avg(self, input, mask):
        x = input * mask
        sum = torch.sum(
            torch.sum(x, dim=2, keepdim=True), dim=3, keepdim=True
        )  # [n,3,1,1]
        mask_sum = torch.sum(
            torch.sum(mask, dim=2, keepdim=True), dim=3, keepdim=True
        )  # [n,1,1,1]
    ...
    def forward(self, fake, real, mask_fake, mask_real):
        ...
        if self.opt.balance_Lab:
            ...
            diff = torch.abs(
                fake_Lab_avg[:, 1:, :, :] - real_Lab_avg[:, 1:, :, :].detach()
            )
            w_diff = weight_mask * diff
...
class StyleContentLoss(nn.Module):
    ...
    def forward(
        ...
        fake_vgg, style_vgg, content_vgg = (
            self.vgg(fake_image),
            self.vgg(style_image),
            self.vgg(content_image),
        )
        if self.opt.remove_background:
            loss_c = self.calc_content_loss(
                fake_vgg[-1], content_vgg[-1].detach(), content_label.detach()
            )
            loss_s = 0
