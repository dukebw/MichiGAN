...
class SPADEBGenerator(BaseNetwork):
    ...
    def forward(
        ...
        if not self.opt.no_orientation:
            if not self.opt.use_ig:
                ...
                orient_input = torch.cat(
                    [torch.sin(2 * orient_mask1), torch.cos(2 * orient_mask1)], dim=1
                )  # [n,2,h,w]
        ...
        hair_mask2 = F.interpolate(
            hair_mask, size=(int(sh / 4), int(sw / 4)), mode="nearest"
        )
        hair_mask3 = F.interpolate(
            hair_mask, size=(int(sh / 8), int(sw / 8)), mode="nearest"
        )
        hair_mask4 = F.interpolate(
            hair_mask, size=(int(sh / 16), int(sw / 16)), mode="nearest"
        )
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        self, opt, ngf=32, norm_layer=nn.InstanceNorm2d, padding_type="reflect"
    ):
        ...
        for i in range(3):
            model_middle += [
                ResnetBlock2(ngf * mult, norm_type="in", padding_type=padding_type)
            ]
...
class SelfAttention(nn.Module):
    def __init__(self, dim, downsample=4):
        ...
        self.query_conv = nn.Conv2d(
            in_channels=dim, out_channels=dim // downsample, kernel_size=1
        )
